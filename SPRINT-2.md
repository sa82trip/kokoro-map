...
- **요구사항**:
  ...
  - 자동 저장 기능 유지
- **태스크**:
  - [ ] 5-1: 스토리지 레이어 리팩토링 — 단일 키 → 다중 문서 관리
  - [ ] 5-2: 문서 메타데이터 모델 정의 (DocumentMeta 타입)
  - [ ] 5-3: FileManagerStore 생성 (문서 CRUD, 활성 문서 관리)
  - [ ] 5-4: JSON 내보내기 유틸리티 (파일 다운로드)
  - [ ] 5-5: JSON 가져오기 유틸리티 (파일 업로드 + 검증)
  - [ ] 5-6: Toolbar에 저장/내보내기/가져오기 버튼 추가
  - [ ] 5-7: 기존 단일 저장 데이터 마이그레이션
...
- **요구사항**:
  ...
  - 문서 삭제 기능
- **태스크**:
  - [ ] 6-1: 홈/시작 화면 컴포넌트 (HomeScreen)
  - [ ] 6-2: 문서 카드 컴포넌트 (DocumentCard — 썸네일, 제목, 날짜)
  - [ ] 6-3: 최근 문서 목록 컴포넌트 (RecentDocumentList)
  - [ ] 6-4: 문서 삭제 기능 (확인 다이얼로그 포함)
  - [ ] 6-5: 라우팅 추가 (홈 ↔ 에디터 전환)
...
- **요구사항**:
  ...
  - 검색 결과 하이라이트
- **태스크**:
  - [ ] 7-1: 검색 입력 컴포넌트 (SearchBar)
  - [ ] 7-2: 검색/필터 로직 구현 (FileManagerStore 확장)
  - [ ] 7-3: 필터 드롭다운 컴포넌트 (날짜별, 정렬)
  - [ ] 7-4: 검색 결과 하이라이트 적용
  - [ ] 7-5: 빈 결과 상태 UI
...
- **요구사항**:
  ...
  - 사이드바 폴더 트리 UI
- **태스크**:
  - [ ] 8-1: 폴더 데이터 모델 정의 (Folder 타입)
  - [ ] 8-2: 폴더 CRUD 로직 (FileManagerStore 확장)
  - [ ] 8-3: 폴더 트리 사이드바 컴포넌트 (FolderTree)
  - [ ] 8-4: 폴더 생성 다이얼로그
  - [ ] 8-5: 문서 이동 기능 (드래그앤드롭 또는 선택 UI)
  - [ ] 8-6: 폴더 컨텍스트 메뉴 (이름 변경, 삭제)
...
## 계획
- **1주차 (3/30 ~ 4/5)**: US-5 파일 저장/불러오기 (핵심 인프라)
- **2주차 (4/6 ~ 4/12)**: US-6 최근 문서 관리 (홈 화면)
- **3주차 (4/13 ~ 4/19)**: US-7 검색/필터링 + US-8 폴더 구조
